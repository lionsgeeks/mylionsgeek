...
export default function GlobalAnalytics({
    ...
    const equipmentByState = useMemo(() => {
        ...
        return [
            { name: 'Functional', value: ok },
            { name: 'Non-Functional', value: bad },
        ];
    }, [equipment]);
        ...
        <AppLayout>
            ...
            <div className="mx-auto max-w-7xl px-4 py-6 sm:p-8 lg:p-10">
                    ...
                    <div className="rounded-xl border border-sidebar-border/70 bg-card p-4">
                        <div className="mb-3 flex items-center justify-between">
                            ...
                            {absent_today.length > 0 && (
                                <button
                                    onClick={() => downloadCSV('absent_today.csv', absent_today)}
                    ...
                    <div className="rounded-xl border border-sidebar-border/70 bg-card p-4">
                        <div className="mb-3 flex items-center justify-between">
                            ...
                            {absent_week.length > 0 && (
                                <button
                                    onClick={() => downloadCSV('absent_week.csv', absent_week)}
                                    className="rounded border px-3 py-1 text-xs hover:bg-muted"
                                >
                                    Export CSV
                                </button>
                    ...
                    <div className="rounded-xl border border-sidebar-border/70 bg-card p-4">
                        <div className="mb-3 flex items-center justify-between">
                            ...
                            {absent_month.length > 0 && (
                                <button
                                    onClick={() => downloadCSV('absent_month.csv', absent_month)}
                                    className="rounded border px-3 py-1 text-xs hover:bg-muted"
                                >
                                    Export CSV
                                </button>
                    ...
                    <div className="rounded-xl border border-sidebar-border/70 bg-card p-4 lg:col-span-3">
                        <div className="mb-3 flex items-center justify-between">
                            ...
                            {most_absent.length > 0 && (
                                <button
                                    onClick={() => downloadCSV('most_absent.csv', most_absent)}
                                    className="rounded border px-3 py-1 text-xs hover:bg-muted"
                                >
                                    Export CSV
                                </button>
                ...
                <div className="mb-10 grid grid-cols-1 gap-6 lg:grid-cols-2">
                    <div className="rounded-xl border border-sidebar-border/70 bg-card p-4">
                        <div className="mb-3 flex items-center justify-between">
                            ...
                            {latest_reservations.length > 0 && (
                                <button
                                    onClick={() => downloadCSV('latest_reservations.csv', latest_reservations)}
                                    className="rounded border px-3 py-1 text-xs hover:bg-muted"
                                >
                                    Export CSV
                                </button>
                            ...
                            {latest_reservations.map((r) => (
                                <div key={r.id} className="flex items-center justify-between rounded-lg border px-3 py-2">
                                    <div className="min-w-0">
                                        <div className="truncate font-medium">{r.title || `Reservation #${r.id}`}</div>
                                        <div className="truncate text-xs text-muted-foreground">
                                            {r.date} {r.start} - {r.end} · {r.type} · {r.user_name}
                                        </div>
                                    </div>
                                    <span
                                        className={`rounded px-2 py-1 text-xs ${r.canceled ? 'bg-red-500/15 text-red-600' : r.approved ? 'bg-green-500/15 text-green-700' : 'bg-amber-500/15 text-amber-700'}`}
                                    >
                                        {r.canceled ? 'Canceled' : r.approved ? 'Approved' : 'Pending'}
                                    </span>
                                </div>
                            ))}
                    ...
                    <div className="rounded-xl border border-sidebar-border/70 bg-card p-4">
                        <div className="mb-3 flex items-center justify-between">
                            ...
                            {equipment.length > 0 && (
                                <button
                                    onClick={() => downloadCSV('equipment.csv', equipment)}
                                    className="rounded border px-3 py-1 text-xs hover:bg-muted"
                                >
                                    Export CSV
                                </button>
                        ...
                        <div className="text-xs text-muted-foreground">
                            In reservation today: {equipment.filter((e) => e.in_reservation_today).length}
                        </div>
                    </div>
                </div>
                ...
                <div className="mb-10 grid grid-cols-1 gap-6 lg:grid-cols-2">
                    <div className="rounded-xl border border-sidebar-border/70 bg-card p-4">
                        <div className="mb-3 flex items-center justify-between">
                            ...
                            {computers.length > 0 && (
                                <button
                                    onClick={() => downloadCSV('computers.csv', computers)}
                                    className="rounded border px-3 py-1 text-xs hover:bg-muted"
                                >
                                    Export CSV
                                </button>
                        ...
                        <div className="space-y-2 text-sm">
                            {computers.map((c) => (
                                <div key={c.id} className="flex items-center justify-between rounded-lg border px-3 py-2">
                                    <div className="min-w-0">
                                        <div className="truncate font-medium">{c.reference || `PC #${c.id}`}</div>
                                        <div className="truncate text-xs text-muted-foreground">{c.mark}</div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span
                                            className={`rounded px-2 py-1 text-xs ${c.state ? 'bg-green-500/15 text-green-700' : 'bg-red-500/15 text-red-600'}`}
                                        >
                                            {c.state ? 'Available' : 'Busy'}
                                        </span>
                                        <span className="max-w-[140px] truncate text-xs text-muted-foreground">{c.user_name || '—'}</span>
                                    </div>
                                </div>
                            ))}
                    ...
                    <div className="rounded-xl border border-sidebar-border/70 bg-card p-4">
                        <div className="mb-3 flex items-center justify-between">
                            ...
                            {projects_new.length > 0 && (
                                <button
                                    onClick={() => downloadCSV('projects_new.csv', projects_new)}
                                    className="rounded border px-3 py-1 text-xs hover:bg-muted"
                                >
                                    Export CSV
                                </button>
                ...
                <div className="flex items-center justify-end">
                    <button onClick={() => window.print()} className="rounded border px-4 py-2 hover:bg-muted">
                        Export PDF (Print)
                    </button>
                </div>
            </div>
