...
const Rolegard = ({ children, authorized = [], except = [] }) => {
    ...
    const hasAuthorizedRole = allowedRoles.length === 0 ? true : userRoles.some((role) => allowedRoles.includes(role));
    ...
    */
    const isAuthorized = (hasAuthorizedRole && !(!allowedRoles.length && hasExcludedRole)) || (allowedRoles.length === 0 && !hasExcludedRole);
